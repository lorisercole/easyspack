...
```
**NOTE** – The user should have write access to these paths.

You typically want to edit Spack's configuration files:
  - [`concretizer.yaml`](share/spack_config/concretizer.yaml) : to reuse installed packages.
  - [`config.yaml`](share/spack_config/config.yaml.tpl) : to set the location of Spack-installed packages.
  - [`modules.yaml`](share/spack_config/modules.yaml.tpl) : to set the location of modules generated by Spack (if needed).
  - [`upstreams.yaml`](share/spack_config/upstreams.yaml.tpl) : to use an upstream database that was generated by spood (only if you generate a db).

A typical Spack user-scope configuration that would work with a system/site installation of Spack can be generated from the templates available in the [`share/spack_config`](share/spack_config/) folder, by replacing `INSTALL_BASE_PATH` with the absolute path of the installation directory (it can be the same as `SPACK_USER_CONFIG_PATH`). To do so, you can run this code:
```bash
...
It consists of these steps:
  ...
  *Example:* [`examples/ext_install/externals_nocompat.yaml`](examples/ext_install/externals_nocompat.yaml) (assuming that a *haswell* microarchitecture was detected by EESSI).
  ...
  - (optional, suggested) detect OS packages available under the EESSI compat-layer, and configure Spack to use them as externals. In EESSI, these packages are often dependencies of software-layer packages, so it is suggested to include them. This can be done with:
    ```bash
    spack external find --all -p "${EESSI_EPREFIX}" --exclude gcc
    spack external find --all -p "${EESSI_EPREFIX}/usr" --exclude gcc
    ```
...
The first time you want to concretize a new spec that is not yet installed, the only packages that Spack may need to install (not build from source) are:
...
![Output of `quick_start.sh`](notes/spood-demo-1.webp)
![Output of `quick_start.sh`](notes/spood-demo-2.webp)
![Output of `quick_start.sh`](notes/spood-demo-2.webp)
...
What happens:
  ...
  - All externals are registered into an upstream DB (e.g. at `/home/lercole/eessi/spack/upstreams/eessi`). Only compilers are added to a `packages.yaml`.


## Approach #2 [legacy/obsolete]: Custom upstream DB via JSON
This older approach uses `SpecLoader` in [spood/legacy/loader.py](spood/legacy/loader.py) to read a dictionary provided by the user (following [spood/legacy/schema.py](spood/legacy/schema.py)), generate concrete specs, inject runtime libs, and writes a full upstream DB. This flow is kept for reference, but it is superseded by the external packages approach.

> ***Example script***: [`upstreamdb_legacy.py`](upstreamdb_legacy.py)\
> Example script for this approach, that uses the [`examples/upstream_db/eessi_example.py`](examples/upstream_db/eessi_example.py) example data.\
