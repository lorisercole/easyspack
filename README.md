...
You typically want to edit Spack's configuration files:
  ...
  - [`upstreams.yaml`](share/spack_config/upstreams.yaml.tpl) : to use an upstream database that was generated by easyspack (only if you generate a db).
...
This older approach uses `SpecLoader` in [easyspack/legacy/loader.py](easyspack/legacy/loader.py) to read a dictionary provided by the user (following [easyspack/legacy/schema.py](easyspack/legacy/schema.py)), generate concrete specs, inject runtime libs, and writes a full upstream DB. This flow is kept for reference, but it is superseded by the external packages approach.
